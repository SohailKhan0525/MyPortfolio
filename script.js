...
const projectData = {
  'house-price': {
    ...
    description: 'Advanced regression model using Scikit-Learn to predict real estate values with high accuracy. Built with Python and deployed on Streamlit for live predictions.\n\nKey Learnings:\n1) Real-world data is messy - Most work happens before modeling. Columns had mixed formats, missing values, and noisy text patterns. Cleaning this properly made model training possible.\n\n2) Feature engineering is critical - Converting Carpet Area strings to standardized square feet and extracting BHK, Current Floor, and Total Floors taught me how domain features can strongly improve model quality.\n\n3) Outlier handling changes model behavior - Using IQR filtering on price showed me that extreme values can distort regression models. Controlled outlier removal improved stability.\n\n4) Categorical encoding must be reproducible - I learned not to rely on ad-hoc encoding. Saving encoded column structure (columns.pkl) is essential so app-time input matches training-time schema.\n\n5) Model comparison beats model guessing - Instead of picking one algorithm directly, I compared Random Forest vs Gradient Boosting with the same split and metrics. This gave an evidence-based model choice.\n\n6) Metrics should be read together - R² alone is not enough. Combining R², MAE, and RMSE gave better understanding of prediction quality and error magnitude.\n\n7) Deployment needs more than a .pkl model - I learned that model deployment also needs metadata artifacts (columns + categories). Without them, UI predictions can break or become inconsistent.\n\n8) Notebook-to-app transition is a separate skill - Building the notebook taught modeling; integrating it into Streamlit taught product thinking: user inputs, validation, transformations, and safe prediction flow.\n\n9) Data pipeline discipline matters - Temporary columns (area_value, area_unit, Price Per Sqft) are useful during engineering, but I learned to drop them cleanly before final training to avoid leakage/confusion.\n\n10) End-to-end ML is about reliability - The biggest learning: a successful ML project is not just "trained model works once," but a repeatable pipeline that can be understood, reused, and deployed.'
  },
  'earthquake': {
