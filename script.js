...
window.addEventListener("load", () => {
  ...
  const interval = setInterval(() => {
    epoch++;
    const loss = lossFor(epoch);
    if (loaderText) loaderText.textContent = `Epoch ${epoch}/${totalEpochs} — loss: ${loss.toFixed(2)}`;
    if (barFill) barFill.style.width = ((epoch / totalEpochs) * 100) + '%';

    if (epoch >= totalEpochs) {
      clearInterval(interval);
      if (loaderText) loaderText.textContent = `Training complete — loss: ${loss.toFixed(2)}`;
      setTimeout(() => {
